<template>
	<div class="_SearchFormFaqModal popup">
			...
			<div class="cells --gap">
				...
				<div class="cell w-8/12 m:w-12/12">
					<div class="popup__header popup__header--m">
						<div class="title-l">FAQ</div>
					</div>
					<div class="text-center" v-if="loading">
						<v-spinner class="text-brand"/>
					</div>
					<template v-else>
						<div v-if="mainInfo && mainInfo.faq_search_locations" v-html="text"></div>
					</template>
				</div>
			</div>
		</div>
...
export default {
	...
	computed: {
		...
		text() {
			return this.mainInfo.faq_search_locations.replace(/\n/g, "<br />").replace(/\u2028/g, ' ');
		}
	},
	async created() {
		this.loading = true;
